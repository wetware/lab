...
@click.option('-k', '--kind',
              help="Plot kind.",
              default="scatter", type=str)
@click.option('-ca', '--cache',
              is_flag=True)
def plot(input_file, ticks, view_size, convergence_threshold, kind, cache):
    convergence_file = converge(input_file, view_size, convergence_threshold, ticks, cache)
    plot_line(convergence_file, convergence_threshold, kind)


def plot_line(input_file, convergence_threshold, kind):
    df = pd.read_csv(input_file)
    for t in convergence_threshold:
        df_t = df[df["threshold"] == t].groupby("nodes", as_index=False).mean()
        colors = df_t.tick.map(lambda x: "r" if x == 0 else "b")
        ax = df_t.plot(x="nodes", y="tick", kind=kind, c=colors)
...
@click.option('-ca', '--cache',
              is_flag=True)
def plot_hist(input_file, nodes, ticks, view_size, interval, convergence_threshold, cache):
    ...
    for n in nodes:
        ...
        data = df.loc[df["tick"] == 1]["references"].values / max_value  # Normalized data
        ...
        an = animation.FuncAnimation(fig, prepare_animation(bar_container), min(ticks, df["tick"].max()),
                                     repeat=True, blit=False, interval=interval)

        for ct in convergence_threshold:
            plt.axhline(y=ct, color="r", linestyle="-")
